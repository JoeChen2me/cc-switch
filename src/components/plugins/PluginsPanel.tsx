...
export function PluginsPanel({ onRefresh }: PluginsPanelProps) {
  ...
  const { data: plugins, isLoading, refetch } = useInstalledPlugins(appType);
  const enableMutation = useEnablePlugin(appType);
  const disableMutation = useDisablePlugin(appType);
  const uninstallMutation = useUninstallPlugin(appType);
  const zipInstallMutation = useInstallPluginFromZip(appType);
  ...
  const handleInstallFromZip = async () => {
    try {
      ...
      toast.success(
        t("plugins.zipInstallSuccess", { count: installed.length }),
      );
    } catch (error) {
      toast.error(t("common.error"), { description: String(error) });
    }
      ...
      >
        <div className="flex items-center justify-between px-4 py-2 border-b border-border-default">
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <Badge
              variant="default"
              className="bg-green-600/90 text-white border-0 text-[10px] px-1.5 py-0 h-4"
            >
          ...
          <div className="flex items-center gap-1">
            <Button
              ...
              disabled={zipInstallMutation.isPending}
            >
        ...
        <TabsContent value={appType} className="flex-1 overflow-y-auto mt-0">
          {isLoading ? (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <Button
                      ...
                      disabled={
                        uninstallMutation.isPending || togglingId !== null
                      }
                      title={t("plugins.uninstall")}
                    >
                      ...
                      {(() => {
                        try {
                          return JSON.stringify(
                            JSON.parse(plugin.pluginJsonRaw!),
                            null,
                            2,
                          );
                        } catch {
